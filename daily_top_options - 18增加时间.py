...
if not os.path.exists(file_name):
    ...
    ws2.append(["Date", "Time","Strong Bullish", "Bullish", "Neutral", "Bearish", "Strong Bearish"])
else:
...
if ws2.max_row >= 3:
    row_now = ws2.max_row
    row_prev = row_now - 1
    prev_score = ws2.cell(row=row_prev, column=score_col_index).value
    curr_score = ws2.cell(row=row_now, column=score_col_index).value
    if isinstance(prev_score, int) and isinstance(curr_score, int):
        if curr_score > prev_score:
            ws2.cell(row=row_now, column=score_col_index).fill = PatternFill(start_color="C6EFCE", fill_type="solid")  # 绿
        elif curr_score < prev_score:
            ws2.cell(row=row_now, column=score_col_index).fill = PatternFill(start_color="FFC7CE", fill_type="solid")  # 红
