...
# ✅ 只在整支期权链完全没有 open interest 时才跳过
        if merged_calls['openInterest'].fillna(0).sum() == 0 or merged_puts['openInterest'].fillna(0).sum() == 0:
            continue
        ...
        call_vo_ratio = round(call_volume / call_oi, 2) if call_oi != 0 else 0
        put_vo_ratio = round(put_volume / put_oi, 2) if put_oi != 0 else 0
...
# 2. 成交活跃度对比
        if call_vo_ratio > put_vo_ratio:
            score += 1
        elif call_vo_ratio < put_vo_ratio:
            score -= 1
